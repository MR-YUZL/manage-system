...
const post_mock = ["设计岗", "开发岗"];

const level_mock = ["低级", "中级", "高级"];

const recruit_mock = ["人事部", "销售部", "开发部"];

const education_mock = ["本科", "大专", "硕士"];
...
const list = (params) => {
  let { current, pageSize, condition } = JSON.parse(params.body);
  let data = [];
  const {
    name, //职位名称
    post, //对应岗位
