<template>
  <div class="tool-bar">
    <div class="item" v-for="(item, index) in tools" :key="index">
      <div v-if="!!item.title && item.type !== 'search'" style="margin-right:10px">{{item.title}}</div>
      ...
      <div v-else-if="item.type === 'compact'">
        <a-input-group compact>
          ...
          <a-input
            ...
            :placeholder="item.placeholder"
            @pressEnter="onSearch"
            @change="compactInputChange($event,item)"
...
export default {
      ...
      this.tools.forEach(item => {
        ...
        if (item.name != null) {
          ...
          this.$set(
            this.valueObj,
            item.name,
            item.defaultName || item.defaultName == ""
              ? item.defaultName
              : null,
  ...
  methods: {
    compactInputChange(e, targetItem) {
      this.valueObj[targetItem.name] = e.target.value;
      // console.log(this.valueObj,targetItem,e.target.value);
      this.$emit("onChange", targetItem.name, e.target.value, this.valueObj); // 返回当前改变的字段名，改变的内容，改变后valueObj的内容
    },
