  ...
  <div class="user_data">
    ...
    <div class="content row_1" v-bind:class="{ expansion: active }">
      <a-icon
        ...
        class="icon"
        @click="showFn"
        v-if="userInfList && userInfList.length"
      />
      <div class="content_item">
        <a-row type="flex" justify="space-between" align="bottom">
          <a-col
            :span="20"
            v-for="(item, index) in userInfList"
            :key="index"
            class="item"
          ...
          >
            <div class="list">
              ...
              <div class="item_right item_1" v-if="item.status == 1">
                受理中
              </div>
              <div class="item_right item_2" v-else-if="item.status == 2">
                已完结
              </div>
            </div>
          </a-col>
...
export default {
  ...
  methods: {
    ...
    getWorkOrderList() {
      this.Request.get(
        "/hfw/workbench/getWorkFlowInfo?guestId=" + this.guestId
      ).then(res => {
    ...
    // 查询分类
    getClassification() {
      this.Request.get("/config/system/findTypeListJson", { type: 1 }).then(
        res => {
    ...
    getSkillGroup() {
      ...
      this.Request.get("/staff/hfwStaffSkillGroups/listJson").then(res => {
        console.log("受理组", res.data.list,this.formObjectCreated.modelList);
        let list = res.data.list;
        list.map(item => {
        ...
        this.formObjectCreated = Object.assign({},this.formObjectCreated)
        this.$forceUpdate();
      });
    },
    getStaffListAll() {
      // 受理人，发起人
      this.Request.get("/staff/hfwStaffSkillGroupsMember/staffList", {}).then(
        res => {
          let list = res.data.list;
          list.map(item => {
    ...
    getFormFields() {
      ...
      this.Request.get("/config/hfwConfigFields/formFieldsJson", {
        state: 2
      }).then(res => {
        ...
        this.$set(this.formObjectCreated, "modelList", [
          ...this.formObjectCreated.modelList
        ]);
        this.$forceUpdate();
        console.log(
          "新的字段表==============================",
          this.formObjectCreated
        );
      });
    }
  }
