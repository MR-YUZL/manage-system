<template>
  <div class="ReplayWorkOrder">
    ...
    <div v-if="info.status != 2">
      <BaseForm
        ref="replayForm"
        :formObject="replayFormObject"
        @formSubmit="replyWorkOrder"
      ></BaseForm>
    </div>
    <Modal
      :currentModal="batchTransmit"
      @toggleModal="toggleModal"
      v-if="batchTransmit.visible"
    >
      <div slot="content">
        <BaseForm
    ...
    <a-modal
      title="完结工单"
      :visible="closeWorkOrderShow"
      v-if="closeWorkOrderShow"
      @cancel="handleCancelCloseOrder"
      @ok="handleWorkOrder(3)"
    >
    ...
    <a-modal
      title="重新开启"
      :visible="reopenWorkOrderShow"
      v-if="reopenWorkOrderShow"
      @cancel="handleReopenOrder"
      @ok="handleWorkOrder(4)"
    >
      <a-textarea
    ...
    replyWorkOrder(data) {
      ...
      this.Request.post("/workflow/follow/saveWorkflowFollow", params).then(
        () => {
          ...
          this.$set(this.replayFormObject, this.replayFormObject);
          this.$emit("updataOrderRecord");
        }
      );
    }
