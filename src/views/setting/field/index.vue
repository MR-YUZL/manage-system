    ...
    <div class="">
      ...
      <div class="box">
        <a-tabs :activeKey="activeKey" @change="clickTabs">
          <a-tab-pane key="1" tab="客户字段设置">
          </a-tab-pane>
          <a-tab-pane key="2" tab="联系人字段设置">
          </a-tab-pane>
          <a-tab-pane key="3" tab="工单字段设置">
          </a-tab-pane>
        </a-tabs>
        <div class="addField"><a-button type="primary" @click="addField">添加字段</a-button></div>
      ...
      <a-modal title="添加字段" width="650px" :visible="addFieldShow">
        <a-form-model 
          ref="addForm" 
          :model="addFormData" 
          :rules="rules"
        ...
        >
          ...
          <div class="addChoice" v-if="addOptionsShow">
            <a-form-model-item 
                :wrapperCol= "{ span: 20 }" 
                :labelCol ="{ span: 4 }" 
                v-for="(it,idx) in addFormData.options" 
                :key="idx">
                <div class="singleChoice">
                  ...
                  <a-form-model-item>
                     <a-checkbox @change="changeDefaultValue(idx)"  v-model="addFormData.options[idx].isDefault" :disabled="addFormData.options[idx].disabled">默认</a-checkbox>
                  </a-form-model-item>
...
export default {
    ...
    methods: {
      getList(){
        ...
        this.Request.post('/config/hfwConfigFields/list',{...params}).then(res => {
      ...
      changeDataType(){
        ...
        if(dataType==2||dataType==3){
          this.addFormData.options = [{ isDefault:false,optionValue:'',disabled:false}]
      ...
      addDomain(){
        this.addFormData.options.push( { isDefault:false,optionValue:'',disabled:false})
      },
      removeDomain(idx){
