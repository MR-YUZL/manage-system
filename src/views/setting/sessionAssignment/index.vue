  ...
  <div class="">
      ...
      <div>
        ...
        <div class="conversationCon">
          <div><a-checkbox v-model="sessionVolumeParams.DATA_80013"></a-checkbox>单个客服同时接待量上限<a-input v-model="sessionVolumeParams.DATA_80014" style="width:100px;"></a-input><span>同时接待量达到上限时，新接入的访客将进入排队队列</span></div>
          ...
          <a-tabs :activeKey="activeKey" @change="clickTabs" type="editable-card" >
            <a-tab-pane key="1" tab="web分配">
              <a-form-model :modal="webParams" ref="channelForm">
                <a-form-model-item>
                  <a-radio-group  v-model="webParams.DATA_80024" @change="changeDistrbRadio">
                ...
                <div  v-if="webParams.DATA_80024 == 0">
                  <a-form-model-item >
                  功能开关1 <a-switch v-model="webParams.appointParams.DATA_80025" @change="switchChange" /> 通过Web的访客根据所选问题将被分配为以下客服/客服组
                  </a-form-model-item>
                  <div class="channalFp">
                    <a-form-model-item>
                ...
                <div  v-if="webParams.DATA_80024 == 1">
                  <a-form-model-item>
                    功能开关 <a-switch v-model="webParams.DATA_80028" /> 通过Web的访客根据所选问题将被分配为以下客服/客服组
                  </a-form-model-item> 
                  <div v-for="(item,index) in webParams.navigationParams.questionList" :key="index">
                    ...
                    <div class="channalFp">
                      <a-form-model-item>
                        <a-select v-model="item.assignmentWay" style="width: 150px" @change="handleWebChange"  placeholder="请选择">
                          <a-select-option :value="1">指定客服 </a-select-option>
                          <a-select-option :value="0">指定客服组</a-select-option>
                        </a-select>
                      </a-form-model-item>
                      <a-form-model-item v-if="item.assignmentWay==0">
                        <a-select v-model="item.assignmentValue" style="width: 300px" mode="multiple" placeholder="请选择">
                          <a-select-option v-for="(item,index) in customerGroupList" :key="index" :value="item.groupId">{{item.groupName}}</a-select-option>
                        </a-select>
                      </a-form-model-item>
            ...
            <a-tab-pane key="3" tab="微信小程序分配">
              <a-form-model :modal="weixinAppletParams"  ref="channelForm">
                ...
                <div class="channalFp" v-if="weixinAppletParams.DATA_80018">
                  <a-form-model-item>
                    <a-select v-model="weixinAppletParams.DATA_80019" style="width: 150px" @change="handleWebChange" placeholder="请选择">
                      <a-select-option :value="1">指定客服 </a-select-option>
                      <a-select-option :value="0">指定客服组</a-select-option>
                    </a-select>
                  </a-form-model-item>
...
export default {
    ...
    data() {
        return {
          ...
          webParams:{
            ...
            // DATA_80025:false,//是否开启WEB指定分配 0未开启、1已开启 
            // DATA_80026:1,//WEB指定分配方式 0客服组、1客服
            // DATA_80027:undefined, //WEB指定分配客服组(客服)
            appointParams:{
              DATA_80025:'',
    ...
    methods: {
       getData(){
         ...
         //DATA_80024 WEB分配策略 0指定分配、1导航分配   DATA_80025是否开启WEB指定分配 0未开启、1已开启    DATA_80026 WEB指定分配方式 0客服组、1客服   DATA_80027  WEB指定分配客服组(客服)
        
        //DATA_80028 是否开启WEB导航分配
        // let params = {
        ...
        // }
        // let params = {
        //   dictionaryCodes:['DATA_80024','DATA_80025','DATA_80026','DATA_80027','DATA_80028']
        // }
        // let params = {
        //   dictionaryCodes:['DATA_80024','DATA_80025','DATA_80026','DATA_80027','DATA_80028']
        // }
        ...
        let params = {
          dictionaryCodes:['DATA_80015','DATA_80016','DATA_80026','DATA_80017']
        }
        console.log(params)
        this.Request.post('/data/dictionary/list',{}).then(res => {
          ...
          data.map( item => {
            if(item.dictionaryCode == 'DATA_80010'){
              this.sessionAssignParams.DATA_80010 = item.dictionaryValue
            }else if(item.dictionaryCode == 'DATA_80011'&&item.dictionaryValue==1){
              this.sessionAssignParams.specialDistri.push('DATA_80011')
            }else if(item.dictionaryCode == 'DATA_80012'&&item.dictionaryValue==1){
              this.sessionAssignParams.specialDistri.push('DATA_80012')
            }else if(item.dictionaryCode == 'DATA_80013'){
              this.sessionVolumeParams.DATA_80013 = item.dictionaryValue==1?true:false
            }else if(item.dictionaryCode == 'DATA_80014'){
              this.sessionVolumeParams.DATA_80014 = item.dictionaryValue
            }
          })
        })
      },
      ...
      getQuestionList(){
        this.Request.post('/navigation/assignment/question/list').then((res)=>{
          ...
          data.map(item=>{
            question.push({
              assignmentValue:item.assignmentValue.split(','),
      ...
      saveChnnalDistribu(){
        ...
        if(key=='1'){
          params = {
            ...
            DATA_80025: webParams.DATA_80025?1:0,
            DATA_80026: webParams.DATA_80026,
            DATA_80027: webParams.DATA_80027.length>0?webParams.DATA_80027.join(','):'',
          }
          if(webParams.DATA_80024 == 1){
            this.saveQuestion()
          }
          console.log('保存的参数',this.webParams,params)
        }else if(key==2){
          let weixinParams=this.weixinParams
          params = {
            DATA_80015:weixinParams.DATA_80015,
            DATA_80016:weixinParams.DATA_80016,
            DATA_80017:weixinParams.DATA_80017.join(','),
          }
           console.log('保存的参数2',this.weixinParams,params)
        }else if(key == 3){
          let weixinAppletParams = this.weixinAppletParams
           params = {
            DATA_80018:weixinAppletParams.DATA_80018,
            DATA_80019:weixinAppletParams.DATA_80019,
            DATA_80020:weixinAppletParams.DATA_80020.join(','),
          }
          console.log('保存的参数3',this.weixinAppletParams,params)
        }
        this.saveField(params)
      }
    }
